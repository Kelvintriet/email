...
var cloudflare_worker_default = {
  async email(message, env, _ctx) {
    ...
    const htmlBody = parsed.html ?? void 0;
    const messageId = parsed.headers.find((h) => h.key.toLowerCase() === "message-id")?.value?.replace(/[<>]/g, "").trim();
    ...
    for (const att of parsed.attachments ?? []) {
      ...
      const putRes = await fetch(uploadUrl, {
        method: "POST",
        // Convex storage uses POST for presigned uploads
        headers: { "Content-Type": att.mimeType ?? "application/octet-stream" },
        body: att.content
        // ArrayBuffer from postal-mime
      });
      if (!putRes.ok) {
    ...
    const ingestRes = await fetch(`${baseUrl}/api/email/ingest`, {
      ...
      body: JSON.stringify({
        ...
        htmlBody,
        messageId,
        attachments: savedAttachments.length ? savedAttachments : void 0
      })
    });
    if (!ingestRes.ok) {
